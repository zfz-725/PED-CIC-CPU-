...
BenchConfig ParseArgs(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--ped-cic-bin") cfg.ped_cic_bin = v(arg);
    ...
    if (cfg.ped_cic_bin.empty() || cfg.pri_bin.empty() || cfg.global_input.empty() ||
        cfg.inst_root.empty() || cfg.work_root.empty()) {
        throw std::runtime_error("必须提供 --ped-cic-bin --pri-bin --global-input --inst-root --work-root");
    }
    return cfg;
}
...
void PrintUsage() {
    std::cout << "用法:\n"
              << "  pri_cpu_benchmark --ped-cic-bin <PED-CIC可执行文件> --pri-bin <PRI可执行文件> "
...
int main(int argc, char** argv) {
    try {
        ...
        for (const auto& inst : SortedSubDirs(cfg.inst_root)) {
            ...
            local_ped_cic_time += RunCommandAndMeasure(cmd);
        }
        ...
        const double ped_cic_total = local_ped_cic_time + pri_time;
        const double overhead = (ped_cic_total - baseline_time) / baseline_time * 100.0;

        std::cout << "========================================\n";
        std::cout << "Baseline PED-CIC Time(s): " << baseline_time << "\n";
        std::cout << "PRI Local PED-CIC Time(s): " << local_ped_cic_time << "\n";
        std::cout << "PRI Coordinator Time(s): " << pri_time << "\n";
        std::cout << "PRI Total Time(s): " << ped_cic_total << "\n";
        std::cout << "Overhead vs PED-CIC(%): " << overhead << "\n";
        return 0;
